...
<script type="text/javascript">
 ...
 {
     let matriculas = JSON.parse(respuesta);
     let i;
     let tbody = "";
     let cuerpo = document.getElementById("tabla").getElementsByTagName("tbody")[0];
     let nuevaFila;
     let total = document.getElementById("total").innerHTML = matriculas.length;
     ...
     for (i = 0; i < 4; i++)
     {
	 if (matriculas[i] === undefined)
	     break;
	 nuevaFila = cuerpo.insertRow(cuerpo.rows.length);	 
	 tbody += "<tr>";
	 tbody += "<td>" + matriculas[i].nombres  + "</td>";
	 tbody += "<td>" + matriculas[i].apellidos + "</td>";
	 tbody += "<td>" + matriculas[i].ciclo + "</td>";
	 tbody += "<td>" + matriculas[i].curso + "</td>";
	 tbody += "<td>" + matriculas[i].fechaCreacionM + "</td>";
	 tbody += "</tr>";
...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Reportes de matrículas de alumnos</h6>
        </div>
        <div class="card-body">

	    <div class="widget-content" >
		
		<form class="needs-validation" action="<?= base_url().'/reportes/matriculas'; ?>" method="post" novalidate>
		    <div class="form-row">
